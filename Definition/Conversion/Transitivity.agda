...
mutual
  ...
  trans~↑ (hd-cong s~u) (hd-cong u~w) = let s~w , Str≡ = trans~↓ s~u u~w in hd-cong s~w , refl (ℕⱼ (wfEq Str≡))
  trans~↑ (tl-cong s~u) (tl-cong u~w) = let s~w , Str≡ = trans~↓ s~u u~w in tl-cong s~w , Str≡
  ...
  transConv↓ (Str-refl x) (Str-refl x₁) = Str-refl x
  transConv↓ (ne x) (ne x₁) =
  ...
  transConv↓Term A≡B (Str-ins x) (Str-ins x₁) =
    Str-ins (proj₁ (trans~↓ x x₁))
  ...
  transConv↓Term A≡B (coiter-cong F<>G s<>w h<>g t<>u) (coiter-cong G<>H w<>z g<>f u<>v) =
    let F≡G = soundnessConv↑ F<>G
        ⊢Γ = wfEq F≡G
        [↑] _ _ D _ _ _ _ = F<>G
        ⊢F = redFirst* D
        F▹▹ℕ≡G▹▹ℕ = Π-cong ⊢F F≡G (refl (ℕⱼ (⊢Γ ∙ ⊢F)))
        F▹▹F≡G▹▹G = Π-cong ⊢F F≡G (W.wkEq (W.step W.id) (⊢Γ ∙ ⊢F) F≡G)
    in coiter-cong (transConv↑ F<>G G<>H) (transConv↑Term F≡G s<>w w<>z) (transConv↑Term F▹▹ℕ≡G▹▹ℕ h<>g g<>f) (transConv↑Term F▹▹F≡G▹▹G t<>u u<>v)
  ...
  transConv↓Term A≡B (ℕ-ins x) (Str-ins x₁) = ⊥-elim (WF.ℕ≢Strⱼ A≡B)
  ...
  transConv↓Term A≡B (Empty-ins x₁) (Str-ins x) = ⊥-elim (WF.Empty≢Strⱼ A≡B)
  ...
  transConv↓Term A≡B (Str-ins x) (ne-ins t u x₂ x₃) = ⊥-elim (WF.Str≢neⱼ x₂ A≡B)
  transConv↓Term A≡B (Str-ins x) (univ x₂ x₃ x₄) = ⊥-elim (WF.U≢Strⱼ (sym A≡B))
  transConv↓Term A≡B (Str-ins x₁) (ℕ-ins x) = ⊥-elim (WF.ℕ≢Strⱼ (sym A≡B))
  transConv↓Term A≡B (Str-ins x₁) (Unit-ins x) = ⊥-elim (WF.Unit≢Strⱼ (sym A≡B))
  transConv↓Term A≡B (Str-ins x₁) (Empty-ins x) = ⊥-elim (WF.Empty≢Strⱼ (sym A≡B))
  transConv↓Term A≡B (Str-ins ([~] A D whnfB ())) (suc-cong x₂)
  transConv↓Term A≡B (Str-ins x) (η-eq x₃ x₄ y y₁ x₅) = ⊥-elim (WF.Str≢Πⱼ A≡B)
  transConv↓Term A≡B (Str-ins x₁) (Σ-η x₂ x₃ x₄ x₅ x₆ x₇) = ⊥-elim (WF.Str≢Σⱼ A≡B)
  transConv↓Term A≡B (Str-ins x₁) (η-unit _ _ _ _) = ⊥-elim (WF.Unit≢Strⱼ (sym A≡B))
  ...
  transConv↓Term A≡B (ne-ins t u x x₁) (Str-ins x₂) = ⊥-elim (WF.Str≢neⱼ x (sym A≡B))
  ...
  transConv↓Term A≡B (univ x x₁ x₂) (Str-ins x₃) = ⊥-elim (WF.U≢Strⱼ A≡B)
  transConv↓Term A≡B (univ x x₁ x₂) (ne-ins t u x₃ x₄) = ⊥-elim (WF.U≢ne x₃ A≡B)
  transConv↓Term A≡B (univ x x₁ x₂) (suc-cong x₃) = ⊥-elim (WF.U≢ℕ A≡B)
  transConv↓Term A≡B (univ x x₁ x₂) (coiter-cong _ _ _ _) = ⊥-elim (WF.U≢Strⱼ A≡B)
  ...
  transConv↓Term A≡B (coiter-cong _ _ _ _) (univ x₁ x₂ x₃) = ⊥-elim (WF.U≢Strⱼ (sym A≡B))
  transConv↓Term A≡B (coiter-cong _ _ _ _) (η-eq x₂ x₃ y y₁ x₄) = ⊥-elim (WF.Str≢Πⱼ A≡B)
  transConv↓Term A≡B (coiter-cong _ _ _ _) (Σ-η x₂ x₃ x₄ x₅ x₆ x₇) = ⊥-elim (WF.Str≢Σⱼ A≡B)
  transConv↓Term A≡B (coiter-cong _ _ _ _) (η-unit _ _ _ _) = ⊥-elim (WF.Unit≢Strⱼ (sym A≡B))
  ...
  transConv↓Term A≡B (η-eq x₁ x₂ y y₁ x₃) (Str-ins _) = ⊥-elim (WF.Str≢Πⱼ (sym A≡B))
  ...
  transConv↓Term A≡B (η-eq x₁ x₂ y y₁ x₃) (coiter-cong _ _ _ _) = ⊥-elim (WF.Str≢Πⱼ (sym A≡B))
  ...
  transConv↓Term A≡B (Σ-η x₁ x₂ x₃ x₄ x₅ x₆) (Str-ins x₇) = ⊥-elim (WF.Str≢Σⱼ (sym A≡B))
  ...
  transConv↓Term A≡B (Σ-η x₁ x₂ x₃ x₄ x₅ x₆) (coiter-cong _ _ _ _) = ⊥-elim (WF.Str≢Σⱼ (sym A≡B))
