...
mutual
  ...
  data _⊢_≡_∷_ (Γ : Con Term) : Term → Term → Term → Set where
    ...
    fun-ext : ∀ {f g F G A} → Γ ⊢ f ∷ Π F ▹ G → Γ ⊢ g ∷ Π F ▹ G
            ...
            → Γ ⊢ f ≡ g ∷ Π F ▹ G
    natrec-eq : ∀ {z s F G} → Γ ∙ ℕ ⊢ F ≡ G → Γ ⊢ z ∷ F [ zero ]
              → Γ ⊢ s ∷ Π ℕ ▹ Π F [ var zero ] ▹ F [ suc (var (suc zero)) ]
              → Γ ⊢ natrec (lam F) z s ≡ natrec (lam G) z s ∷ Π ℕ ▹ F -- C?
    natrec-zero : ∀ {z s F} → Γ ∙ ℕ ⊢ F → Γ ⊢ z ∷ F [ zero ]
                → Γ ⊢ s ∷ Π ℕ ▹ Π F [ var zero ] ▹ F [ suc (var (suc zero)) ]
                → Γ ⊢ natrec (lam F) z s ∘ zero ≡ z ∷ F [ zero ]
    natrec-suc : ∀ {n z s F} → Γ ⊢ n ∷ ℕ → Γ ∙ ℕ ⊢ F → Γ ⊢ z ∷ F [ zero ]
               → Γ ⊢ s ∷ Π ℕ ▹ Π F [ var zero ] ▹ F [ suc (var (suc zero)) ]
               → Γ ⊢ natrec (lam F) z s ∘ suc n
                 ≡ (s ∘ n) ∘ (natrec (lam F {-A?-}) z s ∘ n) ∷ F [ suc n ]

data _⊢_⇒_∷_ (Γ : Con Term) : Term → Term → Term → Set where
  natrec-subst : ∀ {C c g m n} → Γ ∙ ℕ ⊢ C → Γ ⊢ c ∷ C [ zero ]
               → Γ ⊢ g ∷ Π ℕ ▹ Π C [ var zero ] ▹ C [ suc (var (suc zero)) ]
               → Γ ⊢ m ⇒ n ∷ ℕ
               → Γ ⊢ natrec (lam C) c g ∘ m ⇒ natrec (lam C) c g ∘ n ∷ C [ n ]
  natrec-zero : ∀ {C c g} → Γ ∙ ℕ ⊢ C → Γ ⊢ c ∷ C [ zero ]
              → Γ ⊢ g ∷ Π ℕ ▹ Π C [ var zero ] ▹ C [ suc (var (suc zero)) ]
              → Γ ⊢ natrec (lam C) c g ∘ zero ⇒ c ∷ C [ zero ]
  natrec-suc : ∀ {C c g n} → Γ ∙ ℕ ⊢ C → Γ ⊢ c ∷ C [ zero ]
             → Γ ⊢ g ∷ Π ℕ ▹ Π C [ var zero ] ▹ C [ suc (var (suc zero)) ]
             → Γ ⊢ natrec (lam C) c g ∘ suc n
               ⇒ (g ∘ n) ∘ (natrec (lam C) c g ∘ n) ∷ C [ suc n ]
...
data _⊢_⇒*_∷_ (Γ : Con Term) : Term → Term → Term → Set where
  id : ∀ {A t} → Γ ⊢ t ⇒* t ∷ A
  arrow : ∀ {A t t' u} → Γ ⊢ t ⇒ t' ∷ A → Γ ⊢ t' ⇒* u ∷ A

data _⊢_⇒*_ (Γ : Con Term) : Term → Term → Set where
  id : ∀ {A} → Γ ⊢ A ⇒* A
  arrow : ∀ {A A' B} → Γ ⊢ A ⇒ A' → Γ ⊢ A' ⇒* B
