                     ...
                     (λ ⊢Δ [σ]₁ → let [σ] = S.irrelevanceSubst [Γ]₁ [Γ] ⊢Δ ⊢Δ [σ]₁
                                in  convEqTerm₁ (proj₁ ([A']₁ ⊢Δ [σ]₁)) (proj₁ ([A] ⊢Δ [σ]₁)) ([A'≡A] ⊢Δ [σ]₁)
                                                (irrelevanceEqTerm (proj₁ ([A'] ⊢Δ [σ]))
                                                                   (proj₁ ([A']₁ ⊢Δ [σ]₁))
                                                                   ([t≡u] ⊢Δ [σ])))
  fundamentalTermEq (Π-cong {E} {F} {G} {H} ⊢F F≡H G≡E) with fundamental ⊢F | fundamentalTermEq F≡H | fundamentalTermEq G≡E
  ... | [Γ] , [F] | [Γ]₁ , modelsTermEq [U] [F]ₜ [H]ₜ [F≡H]ₜ | [Γ]₂ , modelsTermEq [U]₁ [G]ₜ [E]ₜ [G≡E]ₜ =
    let [U]'  = Uₛ [Γ]
        ...
        [E]ₜ' = S.irrelevanceTermLift {A = U} {F = F} {H = H} {t = E}
                                      [Γ] [F] [H] [F≡H] (λ {Δ} {σ} → [U]₁' {Δ} {σ})
                  (S.irrelevanceTerm {A = U} {t = E} [Γ]₂ ([Γ] ∙ [F])
                                     [U]₁ (λ {Δ} {σ} → [U]₁' {Δ} {σ}) [E]ₜ)
        [F≡H]ₜ' = S.irrelevanceEqTerm {A = U} {t = F} {u = H} [Γ]₁ [Γ] [U] (Uₛ [Γ]) [F≡H]ₜ
        [G≡E]ₜ' = S.irrelevanceEqTerm {A = U} {t = G} {u = E} [Γ]₂ (_∙_ {A = F} [Γ] [F]) [U]₁ (λ {Δ} {σ} → [U]₁' {Δ} {σ}) [G≡E]ₜ
    in  [Γ]
    ,   modelsTermEq
          [U]'
          (Πₜₛ {F} {G} [Γ] [F] (λ {Δ} {σ} → [U]₁' {Δ} {σ}) [F]ₜ' [G]ₜ')
  ...
  fundamentalTermEq (natrec-cong {z} {z'} {s} {s'} {n} {n'} {F} {F'} F≡F' z≡z' s≡s' n≡n') |
    ...
    [Γ]₃ , modelsTermEq [ℕ] [n] [n'] [n≡n'] =
      let sType = Π ℕ ▹ (F ▹▹ F [ suc (var zero) ]↑)
          ...
          [F≡F']' = S.irrelevanceEq {A = F} {B = F'} [Γ] ([Γ]₃ ∙ [ℕ]) [F] [F]' [F≡F']
          [F₀≡F'₀] = substSEq {ℕ} {ℕ} {F} {F'} {zero} {zero}
                               [Γ]₃ [ℕ] [ℕ] [ℕ≡ℕ] [F]' [F']' [F≡F']' [0] [0] [0≡0]
          [F₀≡F'₀]' = S.irrelevanceEq {A = F [ zero ]} {B = F' [ zero ]} [Γ]₃ [Γ]₃ (substS {ℕ} {F} {zero} [Γ]₃ [ℕ] [F]' [0]) [F₀]' [F₀≡F'₀]
          [F₊≡F'₊] = sucCaseCong {F} {F'} [Γ]₃ [ℕ] [F]' [F']' [F≡F']'
          [F₊≡F'₊]' = S.irrelevanceEq {A = sType} {B = s'Type} [Γ]₃ [Γ]₃ (sucCase {F} [Γ]₃ [ℕ] [F]') [F₊]' [F₊≡F'₊]
          [Fₙ≡F'ₙ']' = substSEq {ℕ} {ℕ} {F} {F'} {n} {n'}
                                [Γ]₃ [ℕ] [ℕ] [ℕ≡ℕ] [F]' [F']' [F≡F']' [n] [n'] [n≡n']
