...
fullyReducible' : ∀ {A Γ l} ([A] : Γ ⊩⟨ l ⟩ A)
                → ∃ λ B → Whnf B × Γ ⊢ A ⇒* B
fullyReducible' (U' .⁰ 0<1 ⊢Γ) = U , U , id (U ⊢Γ)
fullyReducible' (ℕ D) = ℕ , ℕ , red D
fullyReducible' (ne' K D neK K≡K) = K , ne neK , red D
fullyReducible' (Π' F G D ⊢F ⊢G A≡A [F] [G] G-ext) = Π F ▹ G , Π , red D
...
fullyReducibleTerm' : ∀ {a A Γ l} ([A] : Γ ⊩⟨ l ⟩ A) → Γ ⊩⟨ l ⟩ a ∷ A / [A]
                → ∃ λ b → Whnf b × Γ ⊢ a ⇒* b ∷ A
fullyReducibleTerm' (U x) (Uₜ A d typeA A≡A [t]) = A , typeWhnf typeA , redₜ d
fullyReducibleTerm' (ℕ x) (ℕₜ n d n≡n natN prop) =
  n , naturalWhnf natN , conv* (redₜ d) (sym (subset* (red x)))
fullyReducibleTerm' (ne x) (neₜ k d (neNfₜ neK ⊢k k≡k)) = k , ne neK , redₜ d
fullyReducibleTerm' (Π x) (Πₜ f d funcF f≡f [f] [f]₁) = f , functionWhnf funcF , redₜ d
