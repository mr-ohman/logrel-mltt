...
sucTerm' : ∀ {l Γ n} ([ℕ] : Γ ⊩⟨ l ⟩ℕ ℕ) → Γ ⊩⟨ l ⟩ n ∷ ℕ / ℕ-intr [ℕ] → Γ ⊩⟨ l ⟩ suc n ∷ ℕ / ℕ-intr [ℕ]
sucTerm' (noemb (ℕ D)) ℕ[ n , [ ⊢t , ⊢u , d ] , natN , prop ] = ℕ[ _ , [ suc ⊢t , suc ⊢t , id (suc ⊢t) ] , suc , ℕ[ n , [ ⊢t , ⊢u , d ] , natN , prop ] ]
...
sucEqTerm' : ∀ {l Γ n n'} ([ℕ] : Γ ⊩⟨ l ⟩ℕ ℕ) → Γ ⊩⟨ l ⟩ n ≡ n' ∷ ℕ / ℕ-intr [ℕ] → Γ ⊩⟨ l ⟩ suc n ≡ suc n' ∷ ℕ / ℕ-intr [ℕ]
sucEqTerm' (noemb (ℕ D)) ℕ≡[ k , k' , [ ⊢t , ⊢u , d ] , [ ⊢t₁ , ⊢u₁ , d₁ ] , t≡u , [k≡k'] , prop ] = ℕ≡[ _ , _ , idRedTerm:*: (suc ⊢t) , idRedTerm:*: (suc ⊢t₁) , suc-cong t≡u , suc , ℕ≡[ k , k' , [ ⊢t , ⊢u , d ] , [ ⊢t₁ , ⊢u₁ , d₁ ] , t≡u , [k≡k'] , prop ] ]
...
sucEqTerm [ℕ] [n≡n'] =
  ...
  in  irrelevanceEqTerm (ℕ-intr (ℕ-elim [ℕ])) [ℕ] (sucEqTerm' (ℕ-elim [ℕ]) [n≡n']')
