...
natrecTerm {F} {z} {s} {n} {Γ} {Δ} {σ} {l} [Γ] [F] [F₀] [F₊] [z] [s] ⊢Δ [σ]
  ...
  let [ℕ] = ℕₛ {l = l} [Γ]
      ...
      [σFₙ]' = proj₁ ([F] ⊢Δ ([σ] , [σn]))
      [σFₙ] = irrelevance' (PE.sym (singleSubstComp n σ F)) [σFₙ]'
      [σF₀] = irrelevance' (PE.sym (singleSubstComp zero σ F))
                           (proj₁ ([F] ⊢Δ ([σ] , [σ0])))
...
natrec-congTerm {F} {F'} {z} {z'} {s} {s'} {n} {m} {Γ} {Δ} {σ} {σ'} {l}
  ...
  let [ℕ] = ℕₛ {l = l} [Γ]
      ...
      [σF₀] = irrelevance' (PE.sym (singleSubstComp zero σ F))
                           (proj₁ ([F] ⊢Δ ([σ] , [σ0])))
      [σ'F₀] = irrelevance' (PE.sym (singleSubstComp zero σ' F))
                            (proj₁ ([F] ⊢Δ ([σ'] , [σ'0])))
...
natrecₛ : ∀ {F z s n Γ} ([Γ] : ⊩ₛ Γ)
          ...
          ([Fₙ] : Γ ⊩ₛ⟨ ¹ ⟩ F [ n ] / [Γ])
        → Γ ⊩ₛ⟨ ¹ ⟩ z ∷ F [ zero ] / [Γ] / [F₀]
        → Γ ⊩ₛ⟨ ¹ ⟩ s ∷ Π ℕ ▹ (F ▹▹ F [ suc (var zero) ]↑) / [Γ] / [F₊]
        → ([n] : Γ ⊩ₛ⟨ ¹ ⟩ n ∷ ℕ / [Γ] / [ℕ])
        → Γ ⊩ₛ⟨ ¹ ⟩ natrec F z s n ∷ F [ n ] / [Γ] / [Fₙ]
...
natrec-congₛ : ∀ {F F' z z' s s' n n' Γ} ([Γ] : ⊩ₛ Γ)
          ...
          ([z] : Γ ⊩ₛ⟨ ¹ ⟩ z ∷ F [ zero ] / [Γ] / [F₀])
          ([z'] : Γ ⊩ₛ⟨ ¹ ⟩ z' ∷ F' [ zero ] / [Γ] / [F'₀])
          ([z≡z'] : Γ ⊩ₛ⟨ ¹ ⟩ z ≡ z' ∷ F [ zero ] / [Γ] / [F₀])
          ([s] : Γ ⊩ₛ⟨ ¹ ⟩ s ∷ Π ℕ ▹ (F ▹▹ F [ suc (var zero) ]↑) / [Γ] / [F₊])
          ([s'] : Γ ⊩ₛ⟨ ¹ ⟩ s' ∷ Π ℕ ▹ (F' ▹▹ F' [ suc (var zero) ]↑) / [Γ]
                           / [F'₊])
          ([s≡s'] : Γ ⊩ₛ⟨ ¹ ⟩ s ≡ s' ∷ Π ℕ ▹ (F ▹▹ F [ suc (var zero) ]↑)
                             / [Γ] / [F₊])
          ([n] : Γ ⊩ₛ⟨ ¹ ⟩ n ∷ ℕ / [Γ] / [ℕ])
          ([n'] : Γ ⊩ₛ⟨ ¹ ⟩ n' ∷ ℕ / [Γ] / [ℕ])
          ([n≡n'] : Γ ⊩ₛ⟨ ¹ ⟩ n ≡ n' ∷ ℕ / [Γ] / [ℕ])
        → Γ ⊩ₛ⟨ ¹ ⟩ natrec F z s n ≡ natrec F' z' s' n' ∷ F [ n ] / [Γ] / [Fₙ]
