...
Whnf-Unit-prop : {t : Term n}
               → Unit-prop Γ t
               → Whnf t
Whnf-Unit-prop {t = .star} starᵣ = starₙ
Whnf-Unit-prop {t = t} (ne (neNfₜ neK ⊢k k≡k)) = ne neK
...
whNormTerm′ (Unitᵣ x) (Unitₜ n d k≡k prop) =
  n , Whnf-Unit-prop prop , convRed:*: d (sym (subset* (red x)))
...
whNormTerm′ (∪ᵣ′ F G D ⊢F ⊢G A≡A [F] [G]) (∪₁ₜ p d p≡p pa i x) =
  p , injectionLWhnf i , convRed:*: d (sym (subset* (red D)))
whNormTerm′ (∪ᵣ′ F G D ⊢F ⊢G A≡A [F] [G]) (∪₂ₜ p d p≡p pa i x) =
