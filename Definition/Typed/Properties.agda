...
wkIndex : ∀ {Γ Δ x A} → (pr : Γ ⊆ Δ) →
        ...
        in  ⊢ Δ' → x ∷ A ∈ Γ → x' ∷ A' ∈ Δ'
wkIndex base ⊢Δ ()
wkIndex (step pr) (⊢Δ ∙ x) i = let r = (wkIndex pr ⊢Δ i) in PE.subst (\ x → _ ∷ x ∈ _) {!!} (there r)
wkIndex (lift pr) (⊢Δ ∙ x) (there i) = PE.subst (\ x → _ ∷ x ∈ _) {!!} (there (wkIndex pr ⊢Δ i))
wkIndex (lift pr) ⊢Δ here = PE.subst (let G = _; n = _ in \ x → n ∷ x ∈ G) {!!} here
